...
, mTimeSinceLastPacket(sf::seconds(0.f)) {
    ...
    context.music->play(Music::MissionTheme);   // Play game theme
}
...
void MultiplayerGameState::draw() {
    ...
    else { mWindow.draw(mFailedConnectionText); }
}

void MultiplayerGameState::onActivate() { mActiveState = true; }
...
bool MultiplayerGameState::handleEvent(const sf::Event& event) {
    ...
    else if (event.type == sf::Event::GainedFocus) { mHasFocus = true; }
    else if (event.type == sf::Event::LostFocus) { mHasFocus = false; }
